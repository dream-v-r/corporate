...
$$in{birthYear}.= " $$in{birthMonth}";
$$in{birthYear}.= " $$in{birthDay}";
$$in{language}.= " $$in{years}";
# 禁止ワードチェック
...
sub prev_form {
	...
	my ($bef,$item,$jflg);
	foreach my $key (@$key) {
		next if ($key eq "firstName");
		next if ($key eq "firstNameKana");
		next if ($key eq "birthMonth");
		next if ($key eq "birthDay");
		next if ($key eq "years");
		next if ($bef eq $key);
		next if ($key eq "x");
		next if ($key eq "y");
		next if ($key eq "need");
...
sub send_mail {
	...
	if ($$in{name}) {
		$$in{name} =~ s/[\r\n]//g;
		$from = mime_unstructured_header("\エンジニアファン運営事務局\ <engineer\@dream-v.co.jp>");
	} else {
		$from = mime_unstructured_header("\エンジニアファン運営事務局\ <engineer\@dream-v.co.jp>");
	}
